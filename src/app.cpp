...
void deinitApp(App &_app)
{
    ...
    endwin();
    config::deinitConfig(_app.config);
}
...
void drawUserInterface(App &_app)
{
    ...
    static const char CONTROLS_STRING[MAX_STRING_SIZE] = "W/A/S/D - poruszanie, F1 - wyjscie z gry, F2 - restart gry";
    ...
    mvwprintw(_app.ui_window, 1, (WINDOW_WIDTH - strlen(GAME_TITLE_STRING)) / 2, GAME_TITLE_STRING);
    mvwprintw(_app.ui_window, WINDOW_USER_INTERFACE_HEIGHT - 2, (WINDOW_WIDTH - strlen(CONTROLS_STRING)) / 2,
              CONTROLS_STRING);
