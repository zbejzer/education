...
bool Board::canWinWithPerfectOpponent(const unsigned char pawn_colour, const unsigned char moves) const
{
	...
	if (!isBoardCorrect() || total_end_pawns_count > size * size || requested_pawn_count + moves < size || isGameOver() != PAWN_EMPTY)
	{
		return false;
	}
	...
	while (!possible_boards.empty() && possible_boards.top().getTotalPawnsCount() < total_end_pawns_count)
	{
		while (possible_boards.top().getTotalPawnsCount() + 1 < total_end_pawns_count)
		{
			...
			current_board.generatePossibleTurns(PAWNS_ORDER[current_board.getTotalPawnsCount() % 2], false, possible_boards);
		}
