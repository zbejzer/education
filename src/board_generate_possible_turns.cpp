        ...
        for (unsigned char row = 0; row < size; row++)
        {
            if (board_red(col, row) == pawn_colour)
            {
                for (unsigned char i = 0; i < 6; i++)
                {
                    ...
                    if (posInRange(test_col, test_row) && board_red(test_col, test_row) == PawnColour::Empty &&
    !pawn_created[test_col][test_row])
                    {
                        Board new_board(*this);
                        new_board.addPawn(pawn_colour, static_cast<unsigned char>(test_col), static_cast<unsigned
    char>(test_row)); pawn_created[test_col][test_row] = true; possible_turns.push(new_board);
                    }
                }
            }
        }
...
void Board::generateAllPossibleTurns(const PawnColour pawn_colour, const bool should_win,
                                     Stack<Board> &possible_turns) const
{
    for (unsigned char col = 0; col < size; col++)
    {
        for (unsigned char row = 0; row < size; row++)
