...
template<typename T>
Array1D<T>::Array1D() : _data(nullptr), _size(0)
{
}

template<typename T>
Array1D<T>::Array1D(size_t size) : _size(size)
{
...
template<typename T>
inline T& Array1D<T>::operator()(const size_t& index)
{
	return _data[index];
}
