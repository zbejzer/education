...
bool Board::addPawnAndCheckForWin(const PawnColour target_pawn_colour, const unsigned char moves, const unsigned char start_col, const unsigned char start_row)
{
	if (moves > 0)
	{
...
bool Board::canWinWithNaiveOpponent(const PawnColour target_pawn_colour, const unsigned char moves)
{
	...
	if (!isBoardCorrect() || total_moves_to_make + getPawnsCount() > size * size || requested_pawns_end_count < size || isGameOver() != PawnColour::Empty)
	{
		return false;
	}
